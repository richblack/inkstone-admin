...
function buildGraph(triplets: Triplet[], entityStats: Map<string, EntityStat>) {
...
export default function Graph() {
  ...
  const [entityStats, setEntityStats] = useState<Map<string, EntityStat>>(new Map());
  ...
  useEffect(() => {
    async function loadInitial() {
      ...
      const statsMap = new Map<string, EntityStat>();
      entities.forEach(e => statsMap.set(e.name, e));
  ...
  // Rebuild graph whenever triplets or entity stats change
  useEffect(() => {
    if (allTriplets.length === 0) return;
    setGraphData(buildGraph(allTriplets, entityStats));
  }, [allTriplets, entityStats]);
        ...
        <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
          <div className="flex items-center gap-3 flex-wrap">
            <div className="text-xs text-zinc-500">
              {graphData.nodes.length} 節點 · {graphData.links.length} 邊
              · 已載入 {allTriplets.length} 筆
              {totalCount > 0 && (
