// Cloudflare Pages Function — MatchGPT API Proxy
...
const MATCHGPT_BASE = 'https://matchgpt.finally.click';

export const onRequest: PagesFunction<Env> = async (context) => {
  const { request, env, params } = context;

  // 取出 /api/matchgpt/ 之後的路徑
  const routeParts = params['route'] as string[] | undefined;
  const routePath = routeParts ? routeParts.join('/') : '';

  const originalUrl = new URL(request.url);
  const targetUrl = `${MATCHGPT_BASE}/${routePath}${originalUrl.search}`;
